...
#  问号：方法名后面加问号是 Ruby 的编码风格规范，用来表示该方法会返回一个 boolean 类型的值，但是并不会强制该方法一定要返回 boolean 值。
#  感叹号： 方法名后面加感叹号是 Ruby 的编码风格规范，用来表示该方法会去修改对象本身，Ruby 会把带有感叹号的方法称为危险的方法，因为程序中可能有其他地方会引用这个对象。但是并不会强制该方法一定要修改对象本身。
